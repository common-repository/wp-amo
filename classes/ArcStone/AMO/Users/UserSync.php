...
use ArcStone\AMO\KeyedSearchCollection;
...
class UserSync
{
  ...
  {
    $this->debug = new DebugLog('Sync Operation');
    $this->errors = new DebugLog('Errors', true);
  }
  ...
  public function sync_users($page_per = 5000) {
    ...
    while( !($returned_count < $page_per)) {
      ...
      $this->debug->add_item('Requesting data from: ' . $requester->getRequestUrl());
      ...
      $this->debug->add_item('Received ' . count($response) . ' records');
      $page_number++;
    }
    $requests_execution_time = (microtime(true) - $requests_start);
    $this->debug->add_item('API Requests took ' . $requests_execution_time);

    $user_processing_start = microtime(true);
    $result = $this->_process_users($users);
    $user_processing_execution_time = (microtime(true) - $user_processing_start);
    $this->debug->add_item('User processing (updates, inserts, deletes) took ' . $user_processing_execution_time . ' seconds');

    // refresh the cache after every sync so that results
    // will immediately appear on the frontend
//    $cache_refresh_start = microtime(true);
//    $api = new API(\ArcStone\AMO\AMO_API_KEY);
//    $api->clearCache();
//    $api->processRequest(IndividualsRequest::$endpoint);
//    $cache_refresh_exec_time = (microtime(true) - $cache_refresh_start);
//    $this->debug->add_item('User data cache refresh took ' . $cache_refresh_exec_time . ' seconds');


    $sync_execution_time = (microtime(true) - $sync_start);
    $this->debug->add_item('Sync total execution took: ' . $sync_execution_time . ' seconds.');
  ...
  private function _process_users(array $users = array()) {
    ...
    $results = array(
      'added' => 0,
      'updated' => 0,
      'deleted' => $this->_purge_deleted_users()
    );

    $amo_pks = array_column($users, 'pk_association_individual');
    $db_users = $this->_retrieve_wp_users($amo_pks);
    ...
    foreach ( $users as $user ) {
      ...
      if ( isset($user_id) && is_wp_error( $user_id ) ) {
        $user_info = $userdata['first_name'] . ' ' . $userdata['last_name'] . ', ' . $userdata['user_email'];
        $this->errors->add_item($user_info . ': ' .$user_id->get_error_message());
        unset($user_id);
      }
    }
  ...
  private function _purge_deleted_users() {
    ...
    foreach ($deleted_users['DATA'] as $user) {
      $wp_user = $this->_find_wp_user($user['PK_ASSOCIATION_INDIVIDUAL']);
      if (!$wp_user) {
        continue;
      }
